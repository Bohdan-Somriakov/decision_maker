...
def rename_criteria(matrix_frame, criteria_entries_list, criteria_entries):
    ...
    for i, label in enumerate(labels_list):
        if i < len(labels_list) / 2:
            label.config(text=f'{criteria_entries_list[i]}', fg='green')  # Change text color to green
        else:
            label.config(text=f'{criteria_entries_list[i - int(len(labels_list) / 2)]}', fg='green')  # Change text color to green


def create_criteria_btn(criteria_entries_list, root, matrix_frame, criteria_entries):
    criteria_entries_list.clear()
    criteria_btn = tk.Button(root, text='Apply changes', command=lambda: rename_criteria(matrix_frame, criteria_entries_list, criteria_entries), bg='blue', fg='white')  # Change button color to blue with white text
...
def main_entry_btn_label(root):
    input_matrix_size_label = tk.Label(root, text='Enter the size of the matrix', fg='#2B262D', bg='#9F496E')  # Change label text color to blue
    input_matrix_size_label.pack()
    matrix_size_entry = tk.Entry(root, width=5, fg='blue', bg='lightyellow')
    ...
    input_matrix_size_btn = tk.Button(root, text='Submit',
                                      command=lambda: main_submit(matrix_size_entry, root, entries,
                                                                  input_matrix_size_btn),
                                      bg='#27ae60', fg='white', padx=10, pady=5,
                                      font=('Arial', 12))  # Change button color to green with white text

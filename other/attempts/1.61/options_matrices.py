...
def calculate_display_results(priority_vectors, options_entries_list):
    ...
    for i in range(len(options_priority_vectors[0])):
        for j in range(len(main_priority_vector)):
            sum_ += options_priority_vectors[j][i] * main_priority_vector[j]
            sums.append(sum_)
        option_result_label = tk.Label(results_window, text=f'{options_entries_list[i]} = {sum_}')
        option_result_label.pack(pady='10')
        sum_ = 0
    largest_num = max(sums)
    winner_result_label = tk.Label(results_window, text=f'Winner: {options_entries_list[sums.index(largest_num)]} = {largest_num}')
    winner_result_label.pack(pady='40')

...
def create_change_criteria(root, matrix_size, criteria_entries_list):
    change_criteria_frame = tk.Frame(root)
    change_criteria_frame.pack(side='top')
    create_criteria_entries(change_criteria_frame, matrix_size, criteria_entries_list)
    create_criteria_labels(change_criteria_frame, matrix_size)


def create_criteria_entries(change_criteria_frame, matrix_size, criteria_entries_list):
    criteria_entries = []
    for i in range(matrix_size):
        criteria_entry = tk.Entry(change_criteria_frame, width=15)
        criteria_entry.insert(0, f'Criteria {i}')

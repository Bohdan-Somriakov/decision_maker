...
def display_sum(n, display_frame, sums):
    ...
    label_sum = tk.Label(sum_frame, text='Sum:' + 8 * ' ', fg='green')
    label_sum.grid(row=0, column=0)

    for i in range(n):
        label = tk.Label(sum_frame, text=f"{round(sums[i], 2):.2f}", width=8, fg='green')
...
def display_eigenvector(eigenvector_frame, eigenvector):
    ...
    eigenvector_label = tk.Label(eigenvector_frame, text='eigenvector', width=10, fg='orange')
    eigenvector_label.grid(row=0, column=0)
    for i in range(len(eigenvector)):
        label_eigenvector_elem = tk.Label(eigenvector_frame, text=f'{round(eigenvector[i], 2):.2f}', width=8, fg='orange')
...
def display_priority_vector(priority_vector_frame, priority_vector):
    ...
    eigenvector_label = tk.Label(priority_vector_frame, text='priority vector', width=10, fg='purple')
    eigenvector_label.grid(row=0, column=0)
    for i in range(len(priority_vector)):
        label_eigenvector_elem = tk.Label(priority_vector_frame, text=f'{round(priority_vector[i], 2):.2f}', width=8, fg='purple')
        label_eigenvector_elem.grid(row=i + 1, column=0)
...
def display_lambda_max(lambda_max, display_frame):
    ...
    label_lambda_max = tk.Label(lambda_max_frame, text='lambda_max:', pady=10, fg='pink')
    label_lambda_max.grid(row=0, column=0)
    for i in range(len(lambda_max)):
        label_lambda_max = tk.Label(lambda_max_frame, text=f'{round(lambda_max[i], 2):.2f}', width=8, fg='pink')
...
def display_variable(frame_title, variable_value, display_frame):
    ...
    title_label = tk.Label(variable_frame, text=f'{frame_title}:', fg='brown')
    title_label.grid(row=0, column=1)

    value_label = tk.Label(variable_frame, text=f'{round(variable_value, 2):.2f}', fg='brown')
